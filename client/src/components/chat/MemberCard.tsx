				...
				<div className="flex items-center space-x-4">
					...
					<div className="flex-1 min-w-0">
						<div
							...
							data-tip={userFromRoom.displayName}
						>
							<p className="text-lg font-medium truncate">
								{userFromRoom.displayName}
							</p>
							{isMe(user, userFromRoom) && <p>(you)</p>}
						</div>
						<span className="text-sm">
							{isRoomOwner(room, userFromRoom) && (
								<p>Role: Owner</p>
							)}
							{!isRoomOwner(room, userFromRoom) &&
								isRoomMod(room, userFromRoom) && (
									<p>Role: Mod</p>
								)}
							{!isRoomOwner(room, userFromRoom) &&
								!isRoomMod(room, userFromRoom) &&
								isRoomMember(room, userFromRoom) && (
									<p>Role: Member</p>
								)}
						</span>
					</div>
					<div className="dropdown dropdown-left">
						<label
							tabIndex={0}
							className="m-1 btn btn-md btn-ghost"
						>
							<TbDots className="text-xl" />
						</label>
						<ul
							tabIndex={0}
							className="p-2 shadow dropdown-content menu bg-base-200 rounded-box w-52"
						>
							{!isMe(user, userFromRoom) && (
								<li>
									<button
										onClick={() =>
											sendMessage(userFromRoom)
										}
									>
										Send message
									</button>
