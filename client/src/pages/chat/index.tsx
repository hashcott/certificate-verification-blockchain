...
const Chat: React.FC<ChatProps> = ({}) => {
	...
	let conversationState = useSelector(
		(state: RootState) => state.conversation
	);
	const { conversations, activeConversation } = conversationState;

	const messagesEndRef = useRef<null | HTMLDivElement>(null);
	const { socket } = useAuthenticatedSocket("ws://localhost:4001/chat");
		...
		if (socket) {
			...
			socket.on("message:created", async (message) => {
				...
				if (
					...
					message.conversation.id === activeConversation.id
				) {
					setMessages((messages: any[]) => [...messages, message]);
				}
			});
			...
			socket.on("room:all", async (rooms) => {
				...
				if (activeRoom) {
					const updated = await rooms.find(
						(room: Room) => room.id === activeRoom.id
					);
			...
			socket.on("conversation:all", async (conversations) => {
				...
				if (activeConversation) {
					const updated = await conversations.find(
						(conversation: any) =>
	...
	const getConversationName = (chat: any) => {
		if (
			chat.recipient.displayName === user.displayName &&
			chat.creator.displayName === user.displayName
		) {
										...
										conversations.map(
											(
												...
												<div
													key={index}
													onClick={() =>
														setActiveConversationAndGetMessages(
															conversation
														)
													}
													className={
								...
								{rooms && activeRoom && (
									<>
										<div className="relative flex items-center p-3 border-b-4 border-base-300">
											...
											<label
												htmlFor="members"
												className={`ml-2 btn btn-sm btn-ghost font-semibold}`}
											>
												<IoIosPeople />{" "}
												<p className="ml-2">Members</p>
											</label>
											{activeRoom.users &&
												isRoomMod(activeRoom, user) && (
													<>
														<label
															htmlFor="edit-chat"
															className={`ml-2 btn btn-sm btn-ghost font-semibold}`}
