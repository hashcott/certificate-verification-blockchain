...
const Reset: React.FC<ResetProps> = ({}) => {
	...
	const submitResetForm = async (
		values: EmailValue,
		helpers: FormikHelpers<EmailValue>
	) => {
		try {
		...
		<>
			...
			<div className="flex w-full m-auto mt-0 shadow-xl lg:w-4/12 md:w-10/12 md:mt-28 bg-base-200 rounded-xl">
				<div className="mx-auto w-96">
					...
					{ApiResponse ? (
						<>
							<p className="m-10 mx-auto text-center text-md">
								Password reset
							</p>
							<p className="m-10 mx-auto text-center text-md">
								{ApiResponse.message}
							</p>
							<p className="m-10 mx-auto font-bold text-center text-md">
								{ApiResponse.email}
							</p>
						</>
					) : (
