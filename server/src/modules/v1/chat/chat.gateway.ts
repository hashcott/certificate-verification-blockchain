		...
		OnModuleInit
{
	...
	public async onMessageAdd(
		...
		@MessageBody()
		data: { message: string; chatId: string; type: 'room' | 'conversation' }
	) {
		try {
			...
			for (const room of socket.rooms) {
				...
				if (
					...
					createdMessage.conversation.id === room
				) {
					this.server.to(room).emit('message:created', createdMessage)
				}
			}
	...
	public async onRoomMembers(@ConnectedSocket() socket: UserSocket) {
		try {
	...
	public async onRoomAll(@ConnectedSocket() socket: UserSocket) {
		try {
			const rooms = await this.roomService.getUserRooms(socket.user.id)
			this.server.to(socket.id).emit('room:all', rooms)
		} catch (err) {
	...
	public async onConversationCreate(
		...
		try {
			const participant = await this.userService.getUserByField(
				'displayName',
				data.participant
			)
			const existingConversation =
				await this.conversationService.findIfExists(
	...
	public async onConversationJoin(
		...
		try {
			...
			const messages =
				await this.messageService.findMessagesForConversation(
					conversationId
				)
	...
	public async onConversationAll(@ConnectedSocket() socket: UserSocket) {
		try {
			const conversations =
				await this.conversationService.getUserConversations(
					socket.user.id
