...
export class RoomController {
	...
	@Get('invitation')
	async getInvitations(@Query('code') code: string) {
		console.log(code)
	...
	@Throttle(60, 60)
	@Get('my/rooms')
	async getUsersRoom(@CurrentUser('id', new ParseUUIDPipe()) userId: string) {
		return this.roomService.getUserRooms(userId)
	}

	@UseGuards(JwtAuthGuard, VerifiedGuard)
	@Post()
	async createRoom(@CurrentUser() user: User, @Body() data: RoomDto) {
